...
const DB = {
  ...
  getVehicleStats(vehicleId) {
    ...
    const fuelEntries  = entries.filter(e => e.type === 'carburant' || e.type === 'recharge');
    const otherEntries = entries.filter(e => e.type !== 'carburant' && e.type !== 'recharge');
    ...
    const totalKwh     = fuelEntries.reduce((s, e)  => s + (parseFloat(e.kwhHome) || 0) + (parseFloat(e.kwhPublic) || 0), 0);
    ...
    return {
      totalOther, totalFuel, totalLiters, totalKwh, purchasePrice,
